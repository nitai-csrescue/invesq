...
- **Known gap**: `gaps[].description` is a verbatim passthrough of the underlying assessment's pillar evidence text (from the separate scoring pipeline, see "AI-powered firm onboarding" below) and is NOT rewritten by the narrative generator — if that upstream evidence names an individual (common for the CS-Leadership pillar), the name still appears in `description` even though every generated field (`p6Recommendation`, `gaps[].impact`/`recommendation`, `pillarSignals`, etc.) is scrubbed of names per the tone policy. Fixing this would mean changing the `scoring.ts` prompt/pipeline that produced the original evidence (a separate, previously-shipped system affecting 137+ existing assessment rows) — treated as an explicit out-of-scope follow-up rather than silently touched here.
