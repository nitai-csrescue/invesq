...
- `GET /api/admin/companies/:id/report-data` returns `AdminCompanyReportData` (`lib/api-spec/openapi.yaml`), nested as `{ reportData, meta }`: `reportData` is the spec-exact `DiagnosticReportData` payload (what actually gets pasted into Claude / stringified in the UI); `meta` (`companyId`, `assessmentDate`, `composite`, `compositeMax`, `tier`) is display-only bookkeeping kept out of the exported JSON so the copy-to-clipboard prompt matches the Notion schema field-for-field.
...
- Derived with zero new Claude calls: `p6Recommendation` ("Retain and Develop" / "Augment" / "Replace", from the p6 CS-Leadership score, appended with that pillar's evidence text when present) and `gaps` (the 3 lowest-scoring pillars, NA treated as 1 so an unscored pillar isn't auto-flagged as worst; `description` falls back to the pillar's static `gapNote` when no Claude evidence narrative exists — true for the 5 migrated legacy tenants, which predate the Claude-scored build pipeline).
- Still blank pending a paid scoring.ts extension + backfill (user-gated, not yet approved): `execSummary`, `compositeContext`, `existingSystems`, `pathForward`, `csHeadcount`, `pillarSignals`, and each gap's `impact`/`recommendation` — none of these have a data source anywhere in the app yet. `preparedForName`/`preparedForTitle` are also left blank (no recipient data exists). `reportDate` defaults to today; `assessmentDate` (in `meta`) is the source assessment's date.
...
- Verified against real data for both migrated and Claude-scored companies: STG/Nomis Solutions (company id 1, migrated legacy tenant, blank evidence → gapNote fallback) — composite 3, tier `Tier 1 · Significant Opportunities`; Mainsail Partners/Fullbay (company id 34, real Claude-scored evidence) — composite 12/16, tier `Tier 3 · Developing`, `p6Recommendation`/`gaps` populated with real citation-backed narrative. Both fetched live via `GET /api/admin/companies/:id/report-data` using a temporary dev-only session (deleted after verification).
