...
`FirmReview.tsx` includes an Export panel (`src/pages/admin/ExportPanel.tsx`) that assembles a `report-data.json` payload matching the real Notion-documented "External CS Diagnostic — Scoring Rubric & Cowork Instructions" (Step 7) schema, then hands the user a ready-to-paste Claude prompt. It does not call Claude directly for the PDF/PPTX itself — but it now generates the report's narrative content via its own Claude pipeline (see "Report export generation pipeline" below), so the payload is fully populated rather than blank.
...
- **Known gap, partially mitigated (2026-07-10)**: `gaps[].description` is a verbatim passthrough of the underlying assessment's pillar evidence text (from the separate scoring pipeline, see "AI-powered firm onboarding" below) and is NOT rewritten by the narrative generator — if that upstream evidence names an individual (common for the CS-Leadership pillar), the name used to leak straight into `description`. A targeted, **render-time-only** redaction (`artifacts/api-server/src/lib/nameRedaction.ts`, applied inside `reportExport.ts`'s shared `toResponse()` funnel) now heuristically strips named individuals out of `gaps[].description` specifically when `gap.title === "CS Leadership"`, replacing them with "the current CS leader". This is a display-time filter, not a data migration: it does not touch the `assessments` table, does not rewrite any of the 137+ existing assessment rows' stored `pX_evidence`, and does not bump `RUBRIC_VERSION` — it applies uniformly to freshly-generated and already-cached `report_exports` rows alike, and to any future consumer that reuses `getReportData`/`getOrGenerateReportExport`. Verified against Fullbay (company 34): a cached `report_exports` row that previously read "CS is led by Kendra Fromm at the Director level..." now serves "CS is led by the current CS leader at the Director level..." with every other field (other gaps, `p6Recommendation`, `pillarSignals`, etc.) byte-identical to before. **Residual, intentionally out of scope**: `pillarEvidence.p6` (and other `pillarEvidence`/`pX` fields) shown separately in `PillarScorecard` (`AdminReportPreview.tsx`) still surface the raw, un-redacted evidence text with the name intact — the redaction was scoped strictly to `gaps[].description` per explicit instruction, not applied repo-wide. Fully closing this would mean either widening the redaction call site or changing the `scoring.ts` prompt/pipeline that produced the original evidence (a separate, previously-shipped system affecting 137+ existing assessment rows) — treated as an explicit follow-up, not silently touched here.
...
- `RUBRIC_VERSION` (currently `"v2"`) is bumped whenever the prompt or output shape changes meaningfully; a bump naturally produces a fresh `report_exports` row on next generation instead of serving stale-format/stale-policy rows as current. Old rows are never mutated or deleted.
