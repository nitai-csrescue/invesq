...
- **Editable pillar evidence**: `PATCH /api/admin/companies/:id/pillar-evidence` (`savePillarEvidence` in `reportExport.ts`) edits `assessments.p{n}_evidence` + mirrors `findings.evidence` (PILLARS order p1=org..p8=ai) in one transaction; em-dashes stripped; does NOT create a revision or reset sign-offs (`loadEffectiveReport` rebuilds `pillarEvidence`/gap descriptions from the live assessment on every read, so no cache bump needed). UI: `PillarEvidenceEditor` in `PortcoReportWorkflow.tsx`, kebab item "Edit pillar evidence" available in every workflow state; sends only dirty fields so untouched P6 (name-redacted in display) is never round-tripped into storage.
- **Editable cover metadata**: Prepared For (name/title/company-line override) and Prepared By (name/org/date) live as 6 nullable columns on `companies` (null = default: blank Prepared For, static `PREPARED_BY` constants, company name, today). `PATCH /api/admin/companies/:id/report-meta` (`updateReportMeta` in `reportExport.ts`) does partial updates; empty string/null clears to default; values are em-dash-stripped. Deliberately does NOT create a revision or reset sign-offs. Effective values ride `meta` (`preparedByName`/`preparedByOrg`/`preparedForCompany` + raw `preparedForCompanyOverride`), keeping `reportData` spec-exact; `reportData.preparedForName/Title/reportDate` come straight from the companies row in `buildBaseReportData`, and `overlayNarrative` preserves them over cached rows so no RUBRIC_VERSION bump is ever needed. UI: `CoverMetaCard` in `PortcoReportWorkflow.tsx` (all non-editing states); PDF cover (`page1Cover.ts`) reads `ctx.meta` (the H1 stays `companyName`).
...
- **Prod schema changes**: the Publish flow automatically diffs the dev database schema against prod and applies the difference (renames need confirmation in the Publish UI). Never add startup-time DDL, deploy-hook pushes, or custom prod migration scripts -- just make sure the dev DB has the change and republish.
- **Prod DB data writes**: the agent cannot write to prod directly. Additive/idempotent DATA fixes ride a startup routine (like the normalizedName backfill). A destructive repair needs a temporary admin endpoint + the two-Publish index dance (see `.agents/memory/prod-data-repair-two-publish.md`).
