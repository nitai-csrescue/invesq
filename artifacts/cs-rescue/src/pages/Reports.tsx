...
type ReportCardKey = "retention" | "expansion" | "playbookImpact" | "ttv";

const PERSONA_REPORT_ORDER: Record<Persona, ReportCardKey[]> = {
  vp: ["retention", "expansion", "playbookImpact", "ttv"],
  cs: ["retention", "playbookImpact", "expansion", "ttv"],
  sales: ["expansion", "retention", "playbookImpact", "ttv"],
  "post-sales": ["ttv", "playbookImpact", "retention", "expansion"],
  support: ["retention", "playbookImpact", "ttv", "expansion"],
  engineering: ["retention", "expansion", "playbookImpact", "ttv"],
  customer: ["retention", "ttv", "playbookImpact", "expansion"],
};

const PERSONA_SUBTITLE: Record<Persona, string> = {
  vp: "Quarterly trends across retention, expansion, time-to-value, and playbook impact.",
  cs: "Your team’s impact: retention saves and playbook performance up top.",
  sales: "Weighted to expansion: pipeline funnel and revenue impact lead.",
  "post-sales": "Onboarding-led: TTV and playbook impact lead the view.",
  support: "Retention-led: how escalations are bending the trend lines.",
  engineering: "Quarterly trends across retention, expansion, time-to-value, and playbook impact.",
  customer: "Outside-in summary: how your account contributes to the trend.",
};
...
export default function Reports() {
  ...
  const order = PERSONA_REPORT_ORDER[persona] ?? PERSONA_REPORT_ORDER.vp;

  const cards: Record<ReportCardKey, React.ReactNode> = {
    retention: (
      <Card key="retention" title="Net retention rate" subtitle="12-week trend">
    ...
    expansion: (
      <Card key="expansion" title="Expansion pipeline" subtitle="Stage funnel · last 90 days">
    ...
    playbookImpact: (
      <Card key="playbookImpact" title="Playbook impact ($k ARR)" subtitle="Saved or expanded by playbook · last 90 days">
        <div className="space-y-2.5">
    ...
    ttv: (
      <Card key="ttv" title="Time-to-value (days)" subtitle="Quarterly median">
        <div className="flex items-end justify-between mb-2">
  ...
  return (
    <div className="p-6 max-w-[1500px] mx-auto" data-testid="reports-page" data-persona={persona}>
      <PageHeader
        eyebrow="Executive reporting"
        title="Reports"
        subtitle={PERSONA_SUBTITLE[persona] ?? PERSONA_SUBTITLE.vp}
      />
