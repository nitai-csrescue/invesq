...
**Implementation note (2026-07-10, Phase 1/2 pass):** `lib/db/src/schema/jobs.ts` was deliberately left untouched by this pass — the `jobs.targetId → jobs.firmId` change sketched above did **not** ship. Risk #4 explicitly flags that making `firmId` `NOT NULL` forecloses any future job type that doesn't target a firm (e.g., a per-company rescore job), and that the job-target model needs a decision *before* this column change ships, not after. That decision was not made as part of this pass, so `jobs` schema/behavior is unchanged; this remains a clearly-scoped follow-up, not something silently dropped.
...
6. **Real incident confirming this isn't a hypothetical (resolves Open Question 2, 2026-07-10):** querying the live "Portfolio Company Diagnostics" Notion database directly turned up an existing page — **EHS Insight** — whose Composite value reads **9** while summing that same page's 8 individual pillar-score properties gives **7**. This is a real, already-existing example of exactly the "stored composite disagrees with a fresh recompute" failure mode Section 3.4 guards against for `report_exports`, just on the Notion side of the boundary instead of the Postgres side — proof this class of bug is not confined to one cache table. Today's `writeDiagnosticToNotion` (above) only ever writes **per-pillar** scores, never a rolled-up Composite property, so this specific drift was not caused by this pipeline (the EHS Insight page's Composite value predates/is external to it — likely a manual entry or a Notion-native formula computed on a different property set). The generalization this incident forces: **recompute-and-reject is not scoped to `report_exports` alone.** Any write path that pushes a stored composite/rollup number into Notion — present (none today) or future (if the mirror is ever extended to also write a Composite property) — must independently recompute that number from the same per-pillar scores being written, compare, and refuse the write (loud log, never a silent Notion-side correction of a page this app doesn't own) on any mismatch, using the identical discipline Section 3.4 already mandates for Postgres-side caches. This becomes a standing rule for every future Notion-write code path, not a one-off fix for EHS Insight's page (which is Notion's own data and out of scope for a Postgres-side schema doc to correct — same boundary already drawn in Risk #5).
...
1. ~~Should `firms.dataAuthority` default flip to `"strict"` automatically once a firm passes the parity gate once, or should that always require an explicit admin action?~~ **Resolved (2026-07-10): explicit admin action only, no auto-promotion.** A clean parity-gate pass proves the data is *currently* structurally sound; it does not prove an admin has reviewed it and is willing to accept fail-loud bootstrap behavior for that firm going forward. `firms.dataAuthority` therefore only ever changes via a deliberate admin toggle — Phase 1/3 ship no code path that flips it automatically, on a gate pass or anything else.
2. ~~Is there a real, previously-observed incident behind "reject mismatches on stored composites" beyond the `report_exports.meta.composite` cache this audit found?~~ **Resolved (2026-07-10): yes — folded into Section 3.5, point 6.** A direct query of the live "Portfolio Company Diagnostics" Notion database found an existing page (**EHS Insight**) whose Composite property (9) disagrees with the sum of its own 8 pillar-score properties (7). The composite-drift risk is not specific to `report_exports` — Section 3.5's Notion redesign now states the recompute-and-reject discipline generalizes to **any** write path that ever pushes a stored composite/rollup value to Notion, not only to the existing per-pillar-only mirror.
3. ~~What is the actual trigger condition for turning on RLS?~~ **Resolved (2026-07-10): the first non-internal login type, not a headcount threshold.** RLS turns on the moment the first authenticated user who is *not* an `@csrescue.com` admin exists — i.e., the first real tenant/client login, of any kind. Counting how many such users exist is not the gate; their mere existence is. Until then, Section 3.3's application-layer `firmId`/`slug` filtering remains the enforcement mechanism, unchanged.
4. Should `findings` fully replace the `assessments.p1..p8` denormalized columns eventually (single source within Postgres itself), or is the two-representation design (fast columns + queryable rows) intended to be permanent? This proposal keeps both indefinitely; a future doc could revisit collapsing to one. *(Still open — not part of this implementation's 4 resolved questions.)*
5. For Phase 2's production parity check — who has already confirmed production's `firms`/`companies`/`assessments` came from the same migration history as dev, versus being seeded independently? If independent, the "compare to dev" framing in Phase 4 doesn't apply and production needs its own from-scratch parity run against whatever its actual origin data was. *(Still open — not part of this implementation's 4 resolved questions.)*
6. ~~Does the eventual Phase 2 ingestion layer need per-connector credentials stored anywhere in Postgres, or does every connector's auth live entirely in Replit-managed secrets?~~ **Resolved (2026-07-10): secrets only, never Postgres.** Every connector's authentication (API keys, OAuth tokens, whatever the connector needs) lives exclusively in Replit-managed secrets/integrations. `ingestion_sources.config` (Section 3.2) holds only non-secret configuration — endpoint URLs, sync cadence, field mappings, connector-specific non-secret IDs. No credential column, encrypted or otherwise, is added to `ingestion_sources` in any phase; this was an assumption in the original draft and is now a firm decision.
